...
export const fabCases: FabCase[] = [
  ...
  },
  {
    ...
    kind: 'mask-lifecycle',
    organization: 'TSMC',
    year: '2018–2020 public program',
    title: 'Dry-cleaning becomes a public EUV mask lifecycle case',
    summary: 'TSMC publicly described an EUV-mask dry-clean technique and reported more than 99% reduction in fall-on particles for the documented use case.',
    whyItMatters: 'It shows that mask lifecycle is not only about imaging: contamination analysis, cleaning and maintaining mask availability are separate manufacturing concerns.',
    claimIds: ['TSMC-EUV-MASK-DRYCLEAN-2020'],
    publicBoundary: 'OpenEUV records TSMC’s published outcome and lifecycle context only. It does not reproduce cleaning chemistry, equipment settings, acceptance thresholds, internal analysis methods or current production instructions.',
    unknowns: ['Current production implementation', 'Private cleanliness and acceptance criteria', 'Detailed cleaning process conditions and internal qualification flow'],
  },
  {
    ...
    organization: 'imec',
    year: '2020',
    title: 'CNT pellicles demonstrate a public reticle-protection path',
    summary: 'imec publicly reported full-field CNT-based pellicles mounted on reticles and exposed in an NXE:3300 EUV scanner, with tested single-pass EUV transmission up to 97%.',
    whyItMatters: 'Pellicles turn contamination protection into an optical tradeoff: the membrane protects the mask while remaining highly transmissive to EUV.',
    claimIds: ['IMEC-CNT-PELLICLE-2020', 'EUV-WAVELENGTH-001'],
    publicBoundary: 'The case is limited to imec’s published research result. OpenEUV does not infer a foundry’s current pellicle choice, production membrane composition, private durability targets or manufacturing recipe.',
    unknowns: ['Current production adoption by individual fabs', 'Private lifetime/defect acceptance criteria', 'Proprietary membrane manufacturing details'],
  },
  {
    id: 'zeiss-aims-euv-qualification',
    kind: 'mask-lifecycle',
    organization: 'ZEISS SMT',
    year: '2023 public overview',
    title: 'EUV mask qualification is a distinct lifecycle function',
    summary: 'ZEISS publicly describes AIMS systems as photomask-qualification tools and states that the AIMS EUV generation can inspect EUV masks.',
    whyItMatters: 'A mask can be manufactured and protected yet still require qualification before wafer exposure; inspection/qualification therefore deserves a separate evidence trail from cleaning and pellicle protection.',
    claimIds: ['ZEISS-AIMS-EUV-MASK-2023'],
    publicBoundary: 'OpenEUV records the published qualification function only. It does not infer proprietary inspection algorithms, customer acceptance thresholds, defect disposition rules or private fab workflows.',
    unknowns: ['Customer-specific qualification criteria', 'Private defect-classification thresholds', 'Internal inspection recipes and disposition workflow'],
  },
  {
