...
export function ScannerScene({ selected, exploded, onSelect }: { selected: Subsystem; exploded: number; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    moduleConfig.forEach((config) => { const mesh = new THREE.Mesh(new THREE.BoxGeometry(config.size[0], config.size[1], config.size[2]), material(config.id)); mesh.position.set(config.base[0], config.base[1], config.base[2]); mesh.castShadow = true; mesh.userData.id = config.id; root.add(mesh); objects.set(config.id, mesh) })
