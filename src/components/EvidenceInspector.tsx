...
export function EvidenceInspector({ subsystemId, nodeName, onNodeSelect }: { subsystemId: string; nodeName?: string | null; onNodeSelect?: (nodeName: string | null) => void }) {
  ...
  return (
    <section className="evidence-inspector" data-evidence-inspector data-subsystem={subsystemId} data-node={nodeName ?? ''}>
      ...
      {availableNodes.length > 0 && <div className="concept-node-nav" aria-label="Concept asset nodes"><button className={!nodeName ? 'active' : ''} onClick={() => onNodeSelect?.(null)}>Subsystem</button>{availableNodes.map((name) => <button key={name} data-evidence-node={name} className={nodeName === name ? 'active' : ''} onClick={() => onNodeSelect?.(name)}>{name}</button>)}</div>}
